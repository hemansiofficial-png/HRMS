  ...
  const fetchEvents = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Fetched lifecycle events:', data);
        console.log('RESIGNATION events:', data.filter((e: any) => e.eventType === 'RESIGNATION'));
        console.log('EXIT events:', data.filter((e: any) => e.eventType === 'EXIT'));
        setEvents(Array.isArray(data) ? data : []);
      } else {
        console.error('Failed to fetch lifecycle events:', response.statusText);
        setEvents([]);
      }
    } catch (error) {
